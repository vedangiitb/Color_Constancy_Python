...
def reduce_brightness(image, factor):
    # Convert the image to float32 format for numerical operations
    image_float = image.astype(float)

    # Reduce the brightness by subtracting the factor from each pixel value
    reduced_image_float = image_float - factor

    # Clip the pixel values to ensure they stay within the valid range
    reduced_image_float = np.clip(reduced_image_float, 0, 255)

    # Convert the float32 image back to uint8 format for display
    reduced_image = reduced_image_float.astype(np.uint8)
...
enhanced_output.save("dark-image.jpg")
img1 = cv2.imread("dark-image.jpg")
# brightness_factor = 50
# img1 = reduce_brightness(img1, brightness_factor)
...
# a = find_color_card(raw)
# b = find_color_card(img1)


while goOn==False and newWidth<=width0*8:
